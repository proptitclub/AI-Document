...
cdef np.ndarray[DTYPE_t, ndim=4] add_pad(np.ndarray[DTYPE_t, ndim=4] X, int pad):
    """
    add padding to X
    ...
    """
    cdef np.ndarray X_pad =np.pad(X,((0,0),(pad,pad),(pad,pad),(0,0)),'constant',constant_values = 0)
    return X_pad.astype(float)


cdef float move_conv2d(np.ndarray conv2d_,np.ndarray w,np.ndarray bias=0):
    cdef np.ndarray s=np.multiply(conv2d_,w)
    cdef float Z=np.sum(s,axis=None)+bias
    return Z

cdef np.ndarray cal_conv2d(np.ndarray A_pre,np.ndarray W,np.ndarray bias,int strides,int pad):
    '''
    ...
    cdef int new_H=np.int((n_H-f+pad*2)/(strides))+1
    cdef int new_W=np.int((n_W-f+pad*2)/(strides))+1
    
    np.ndarray Z=np.zeros((m,new_H,new_W,n_D_cur))
    
    np.ndarray A_with_pading=add_pad(A_pre,pad)
    # print(A_pre.shape,A_with_pading.shape)
    cdef in i,h,w,c
    for i in range(m):
         # duyet qua m image
        np.ndarray image_here=A_with_pading[i,:,:,:]
        for h in range(new_H):
            for w in range(new_W):
                for c in range(n_D_cur):
                    cdef int x_s=h*strides
                    cdef int x_e=h*strides+f
                    cdef int y_s=w*strides
                    cdef int y_e=w*strides+f
                    #print(x_s,x_e,y_s,y_e,image_here.shape,A_with_pading.shape)
                    Z[i,h,w,c]=move_conv2d(image_here[x_s:x_e,y_s:y_e,:],W[:,:,:,c],bias[:,:,:,c])
                    
    return Z
    
cdef np.ndarray cal_pool(np.ndarray A_prev,  string mode = "max",int kernel_size,int strides):
    (m,n_Hp,n_Wp,n_Dp)=A_prev.shape
    
    cdef int f=kernel_size
